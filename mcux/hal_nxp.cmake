...
function(include_driver_ifdef feature_toggle directory module)
  ...
  if(${extra_count} GREATER 0)
    list(GET extra_args 0 driver_lib)
    # Override the MCUX SDK project name to be the target driver lib
    set(sdk_proj ${MCUX_SDK_PROJECT_NAME})
    set(MCUX_SDK_PROJECT_NAME ${driver_lib})
    if (${${feature_toggle}})
      list(APPEND CMAKE_MODULE_PATH
    ...
    # Restore old SDK project name variable
    set(MCUX_SDK_PROJECT_NAME ${sdk_proj})
  elseif(${${feature_toggle}})
    list(APPEND CMAKE_MODULE_PATH
        ${CMAKE_CURRENT_LIST_DIR}/mcux-sdk/drivers/${directory}
    )
    zephyr_include_directories(${CMAKE_CURRENT_LIST_DIR}/mcux-sdk/drivers/${directory})
...
include_driver_ifdef(CONFIG_ADC_MCUX_LPADC		lpadc		driver_lpadc	drivers__adc)
include_driver_ifdef(CONFIG_COUNTER_MCUX_CTIMER		ctimer		driver_ctimer	drivers__counter)
include_driver_ifdef(CONFIG_COUNTER_MCUX_LPC_RTC	lpc_rtc		driver_lpc_rtc	drivers__counter)
include_driver_ifdef(CONFIG_DMA_MCUX_LPC		lpc_dma		driver_lpc_dma	drivers__dma)
include_driver_ifdef(CONFIG_GPIO_MCUX_LPC		lpc_gpio        driver_lpc_gpio drivers__gpio)
include_driver_ifdef(CONFIG_NXP_PINT			pint    	driver_pint	drivers__interrupt_controller)
include_driver_ifdef(CONFIG_NXP_PINT			inputmux	driver_inputmux	drivers__interrupt_controller)
include_driver_ifdef(CONFIG_I2C_MCUX_FLEXCOMM		flexcomm	driver_flexcomm_i2c drivers_i2c)
include_driver_ifdef(CONFIG_I2C_MCUX_FLEXCOMM		flexcomm	driver_flexcomm drivers__i2c)
include_driver_ifdef(CONFIG_I2S_MCUX_FLEXCOMM		flexcomm	driver_flexcomm_i2s drivers__i2s)
include_driver_ifdef(CONFIG_I2S_MCUX_FLEXCOMM		flexcomm	driver_flexcomm drivers__i2s)
include_driver_ifdef(CONFIG_MCUX_OS_TIMER		ostimer		driver_ostimer	drivers__timer)
include_driver_ifdef(CONFIG_PWM_MCUX_SCTIMER		sctimer		driver_sctimer	drivers__pwm)
include_driver_ifdef(CONFIG_SOC_FLASH_LPC		flashiap	driver_flashiap	drivers__flash)
include_driver_ifdef(CONFIG_SPI_MCUX_FLEXCOMM		flexcomm	driver_flexcomm_spi drivers__spi)
include_driver_ifdef(CONFIG_SPI_MCUX_FLEXCOMM		flexcomm	driver_flexcomm	drivers__spi)
include_driver_ifdef(CONFIG_UART_MCUX_FLEXCOMM		flexcomm	driver_flexcomm_usart drivers__spi)
include_driver_ifdef(CONFIG_UART_MCUX_FLEXCOMM		flexcomm	driver_flexcomm	drivers__serial)
include_driver_ifdef(CONFIG_WDT_MCUX_WWDT		wwdt		driver_wwdt	drivers__watchdog)
include_driver_ifdef(CONFIG_ADC_MCUX_ADC12		adc12		driver_adc12	drivers__adc)
include_driver_ifdef(CONFIG_ADC_MCUX_ADC16		adc16		driver_adc16	drivers__adc)
include_driver_ifdef(CONFIG_IPM_IMX_REV2		mu		driver_mu	drivers__ipm)
include_driver_ifdef(CONFIG_CAN_MCUX_FLEXCAN		flexcan		driver_flexcan	drivers__can)
include_driver_ifdef(CONFIG_CAN_MCUX_FLEXCAN_FD		flexcan		driver_flexcan	drivers__can)
include_driver_ifdef(CONFIG_COUNTER_MCUX_PIT		pit		driver_pit	drivers__counter)
include_driver_ifdef(CONFIG_COUNTER_MCUX_RTC		rtc		driver_rtc	drivers__counter)
include_driver_ifdef(CONFIG_DAC_MCUX_DAC		dac		driver_dac	drivers__dac)
include_driver_ifdef(CONFIG_DAC_MCUX_DAC32		dac32		driver_dac32	drivers__dac)
include_driver_ifdef(CONFIG_DMA_MCUX_EDMA		edma		driver_edma	drivers__dma)
include_driver_ifdef(CONFIG_DMA_MCUX_EDMA		dmamux		driver_dmamux	drivers__dma)
include_driver_ifdef(CONFIG_ENTROPY_MCUX_RNGA		rnga		driver_rnga	drivers__entropy)
include_driver_ifdef(CONFIG_ENTROPY_MCUX_TRNG		trng		driver_trng	drivers__entropy)
include_driver_ifdef(CONFIG_ENTROPY_MCUX_CAAM		caam		driver_caam	drivers__entropy)
include_driver_ifdef(CONFIG_ETH_MCUX			enet		driver_enet	drivers__ethernet)
include_driver_ifdef(CONFIG_HAS_MCUX_SMC		smc		driver_smc)
include_driver_ifdef(CONFIG_I2C_MCUX			i2c		driver_i2c	drivers__i2c)
include_driver_ifdef(CONFIG_I2C_MCUX_LPI2C		lpi2c		driver_lpi2c	drivers__i2c)
include_driver_ifdef(CONFIG_I3C_MCUX			i3c		driver_i3c	drivers__i3c)
include_driver_ifdef(CONFIG_MCUX_ACMP			acmp		driver_acmp	drivers__sensor__mcux_acmp)
include_driver_ifdef(CONFIG_PWM_MCUX_FTM		ftm     	driver_ftm	drivers__pwm)
include_driver_ifdef(CONFIG_PWM_MCUX_TPM		tpm		driver_tpm	drivers__pwm)
include_driver_ifdef(CONFIG_PWM_MCUX_PWT		pwt		driver_pwt	drivers__pwm)
include_driver_ifdef(CONFIG_RTC_MCUX			rtc		driver_rtc	drivers__counter)
include_driver_ifdef(CONFIG_SPI_MCUX_DSPI		dspi		driver_dspi	drivers__spi)
include_driver_ifdef(CONFIG_SPI_MCUX_LPSPI		lpspi		driver_lpspi	drivers__spi)
include_driver_ifdef(CONFIG_UART_MCUX			uart		driver_uart	drivers__serial)
include_driver_ifdef(CONFIG_UART_MCUX_LPSCI		lpsci		driver_lpsci	drivers__serial)
include_driver_ifdef(CONFIG_UART_MCUX_LPUART		lpuart		driver_lpuart	drivers__serial)
include_driver_ifdef(CONFIG_WDT_MCUX_WDOG		wdog		driver_wdog	drivers__watchdog)
include_driver_ifdef(CONFIG_WDT_MCUX_WDOG32		wdog32		driver_wdog32	drivers__watchdog)
include_driver_ifdef(CONFIG_COUNTER_MCUX_GPT		gpt		driver_gpt	drivers__counter)
include_driver_ifdef(CONFIG_MCUX_GPT_TIMER		gpt		driver_gpt	drivers__timer)
include_driver_ifdef(CONFIG_COUNTER_MCUX_PIT		pit		driver_pit	drivers__counter)
include_driver_ifdef(CONFIG_COUNTER_MCUX_QTMR		qtmr_1		driver_qtmr_1	drivers__counter)
include_driver_ifdef(CONFIG_DISPLAY_MCUX_ELCDIF		elcdif		driver_elcdif	drivers__display)
include_driver_ifdef(CONFIG_DISPLAY_MCUX_DCNANO_LCDIF	lcdif		driver_lcdif	drivers__display)
include_driver_ifdef(CONFIG_MCUX_PXP			pxp		driver_pxp	drivers__dma)
include_driver_ifdef(CONFIG_GPIO_MCUX_IGPIO		igpio		driver_igpio	drivers__gpio)
include_driver_ifdef(CONFIG_I2C_MCUX_LPI2C		lpi2c		driver_lpi2c	drivers__i2c)
include_driver_ifdef(CONFIG_I2S_MCUX_SAI		sai		driver_sai	drivers__i2s)
include_driver_ifdef(CONFIG_MEMC_MCUX_FLEXSPI		flexspi		driver_flexspi	drivers__memc)
include_driver_ifdef(CONFIG_PWM_MCUX			pwm		driver_pwm	drivers__pwm)
include_driver_ifdef(CONFIG_SPI_MCUX_LPSPI		lpspi		driver_lpspi	drivers__spi)
include_driver_ifdef(CONFIG_UART_MCUX_LPUART		lpuart		driver_lpuart	drivers__serial)
include_driver_ifdef(CONFIG_VIDEO_MCUX_CSI		csi		driver_csi	drivers__video)
include_driver_ifdef(CONFIG_WDT_MCUX_IMX_WDOG		wdog01		driver_wdog01	drivers__watchdog)
include_driver_ifdef(CONFIG_DMA_MCUX_LPC		lpc_dma		driver_lpc_dma	drivers__dma)
include_driver_ifdef(CONFIG_MEMC_MCUX_FLEXSPI		flexspi		driver_flexspi	drivers__memc)
include_driver_ifdef(CONFIG_HAS_MCUX_RDC		rdc		driver_rdc)
include_driver_ifdef(CONFIG_UART_MCUX_IUART		iuart		driver_iuart	drivers__serial)
include_driver_ifdef(CONFIG_ADC_MCUX_12B1MSPS_SAR	adc_12b1msps_sar	driver_adc_12b1msps_sar drivers__adc)
include_driver_ifdef(CONFIG_HWINFO_MCUX_SRC		src		driver_src	drivers__hwinfo)
include_driver_ifdef(CONFIG_HWINFO_MCUX_SIM		sim		driver_sim	drivers__hwinfo)
include_driver_ifdef(CONFIG_HWINFO_MCUX_RCM		rcm		driver_rcm	drivers__hwinfo)
include_driver_ifdef(CONFIG_IPM_MCUX			mailbox		driver_mailbox	drivers__ipm)
include_driver_ifdef(CONFIG_COUNTER_MCUX_SNVS		snvs_hp		driver_snvs_hp	drivers__counter)
include_driver_ifdef(CONFIG_COUNTER_MCUX_SNVS_SRTC	snvs_lp		driver_snvs_lp	drivers__counter)
include_driver_ifdef(CONFIG_COUNTER_MCUX_LPTMR		lptmr		driver_lptmr	drivers__counter)
include_driver_ifdef(CONFIG_MCUX_LPTMR_TIMER		lptmr		driver_lptmr	drivers__timer)
include_driver_ifdef(CONFIG_IMX_USDHC			usdhc		driver_usdhc	drivers__sdhc)
include_driver_ifdef(CONFIG_MIPI_DSI_MCUX		mipi_dsi_split	driver_mipi_dsi_split	drivers__mipi_dsi)
include_driver_ifdef(CONFIG_MIPI_DSI_MCUX_2L		mipi_dsi	driver_mipi_dsi	drivers__mipi_dsi)
include_driver_ifdef(CONFIG_ADC_LPC_ADC			lpc_adc		driver_lpc_adc	drivers__adc)
include_driver_ifdef(CONFIG_MCUX_SDIF			sdif		driver_sdif	drivers__sdhc)
include_driver_ifdef(CONFIG_ADC_MCUX_ETC		adc_etc		driver_adc_etc	drivers__adc)
include_driver_ifdef(CONFIG_MCUX_XBARA			xbara		driver_xbara)
include_driver_ifdef(CONFIG_QDEC_MCUX			enc		driver_enc	drivers__sensor__qdec_mcux)
include_driver_ifdef(CONFIG_CRYPTO_MCUX_DCP			dcp		driver_dcp drivers__crypto)
include_driver_ifdef(CONFIG_DMA_MCUX_SMARTDMA		smartdma	driver_lpc_smartdma drivers__dma)
include_driver_ifdef(CONFIG_DAC_MCUX_LPDAC			dac_1		driver_dac_1 drivers__dac)
