...
#define SCG_FIRCCFG_RANGE_VAL ((SCG0->FIRCCFG & SCG_FIRCCFG_RANGE_MASK) >> SCG_FIRCCFG_RANGE_SHIFT)
...
status_t CLOCK_InitSysOsc(const scg_sosc_config_t *config)
{
    ...
    SCG0->SOSCCSR = (uint32_t)config->enableMode | SCG_SOSCCSR_SOSCEN_MASK;

    CLOCK_SetSysOscMonitorMode(config->monitorMode);

    /* Wait for SOSC clock to be valid. */
    while ((SCG0->SOSCCSR & SCG_SOSCCSR_SOSCVLD_MASK) != SCG_SOSCCSR_SOSCVLD_MASK)
    {
    }
...
{
    if ((SCG0->SOSCCSR & SCG_SOSCCSR_SOSCVLD_MASK) == SCG_SOSCCSR_SOSCVLD_MASK) /* System OSC clock is valid. */
    {
...
status_t CLOCK_InitSirc(const scg_sirc_config_t *config)
{
    ...
    SCG0->SIRCCSR = (uint32_t)config->enableMode;
...
{
    if ((SCG0->SIRCCSR & SCG_SIRCCSR_SIRCVLD_MASK) == SCG_SIRCCSR_SIRCVLD_MASK) /* SIRC is valid. */
    {
...
status_t CLOCK_InitFirc(const scg_firc_config_t *config)
{
    ...
    SCG0->FIRCCFG = SCG_FIRCCFG_RANGE(config->range);
    ...
    if (config->trimConfig != NULL)
    {
        ...
        {
            SCG0->FIRCSTAT = SCG_FIRCSTAT_TRIMFINE(config->trimConfig->trimFine);
        }

        /* Set trim mode. */
        SCG0->FIRCCSR = (uint32_t)config->trimConfig->trimMode;
    ...
    SCG0->FIRCCSR |= (SCG_FIRCCSR_FIRCEN_MASK | (uint32_t)config->enableMode);

    /* Wait for FIRC clock to be valid. */
    while ((SCG0->FIRCCSR & SCG_FIRCCSR_FIRCVLD_MASK) != SCG_FIRCCSR_FIRCVLD_MASK)
    {
    }
...
status_t CLOCK_DeinitFirc(void)
{
    ...
    SCG0->FIRCCSR = SCG_FIRCCSR_FIRCERR_MASK;
...
uint32_t CLOCK_GetFircFreq(void)
{
    ...
    if ((SCG0->FIRCCSR & SCG_FIRCCSR_FIRCVLD_MASK) == SCG_FIRCCSR_FIRCVLD_MASK) /* FIRC is valid. */
    {
...
status_t CLOCK_InitRfFro192M(const fro192m_rf_clk_config_t *config)
{
    ...
    FRO192M0->FROCCSR = FRO192M_FROCCSR_POSTDIV_SEL(config->range);
    FRO192M0->FRODIV  = FRO192M_FRODIV_FRODIV(config->apb_rfcmc_div);
...
uint32_t CLOCK_GetRfFro192MFreq(void)
{
    ...
    if ((FRO192M0->FROCCSR & FRO192M_FROCCSR_VALID_MASK) == FRO192M_FROCCSR_VALID_MASK)
    {
