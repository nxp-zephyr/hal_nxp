...
status_t FLASH_Program(flash_config_t *config, FMU_Type *base, uint32_t start, uint32_t *src, uint32_t lengthInBytes)
{
    ...
    status = flash_check_param(config, base, start, lengthInBytes, FSL_FEATURE_FLASH_BLOCK_PHRASE_SIZE);
    if (status == kStatus_FLASH_Success)
    {
        /* Align length to whole phrase */
        uint32_t alignedLength = ALIGN_DOWN(lengthInBytes, sizeof(uint8_t) * FSL_FEATURE_FLASH_BLOCK_PHRASE_SIZE);
        ...
        if (alignedLength > 0U)
        {
            ...
            while (start <= endAddress)
            {
                status = BOOTLOADER_API_TREE_POINTER->flashDriver->flash_program_phrase(
                    config, base, start, srcWord, FSL_FEATURE_FLASH_BLOCK_PHRASE_SIZE);
                ...
                else
                {
                    ...
                    start += FSL_FEATURE_FLASH_BLOCK_PHRASE_SIZE;
                    srcWord += 4U;
                }
            }
        }
        ...
        if ((kStatus_FLASH_Success == status) && (extraBytes > 0U))
        {
            ...
            status = BOOTLOADER_API_TREE_POINTER->flashDriver->flash_program_phrase(
                config, base, start, extraData, FSL_FEATURE_FLASH_BLOCK_PHRASE_SIZE);
        }
    }
    else
    {
    ...
    flash_config_t *config, FMU_Type *base, uint32_t start, uint32_t *src, uint32_t lengthInBytes)
{
    ...
    status = flash_check_param(config, base, start, lengthInBytes, FSL_FEATURE_FLASH_BLOCK_PAGE_SIZE);
    if (status == kStatus_FLASH_Success)
    {
        /* Align length to whole phrase. */
        uint32_t alignedLength = ALIGN_DOWN(lengthInBytes, sizeof(uint8_t) * FSL_FEATURE_FLASH_BLOCK_PAGE_SIZE);
        uint32_t extraBytes    = lengthInBytes - alignedLength;
        uint32_t *srcWord      = (uint32_t *)(uintptr_t)src;
        ...
        if (alignedLength > 0U)
        {
            ...
            while (start <= endAddress)
            {
                status = BOOTLOADER_API_TREE_POINTER->flashDriver->flash_program_page(
                    config, base, start, srcWord, FSL_FEATURE_FLASH_BLOCK_PAGE_SIZE);
                if (kStatus_FLASH_Success != status)
                {
                    break;
                }
                else
                {
                    ...
                    start += FSL_FEATURE_FLASH_BLOCK_PAGE_SIZE;
                    srcWord += 32U;
                }
            }
        }
        ...
        if ((kStatus_FLASH_Success == status) && (extraBytes > 0U))
        {
            ...
            status = BOOTLOADER_API_TREE_POINTER->flashDriver->flash_program_page(config, base, start, extraData,
                                                                                  FSL_FEATURE_FLASH_BLOCK_PAGE_SIZE);
        }
    }
    else
    {
    ...
    flash_config_t *config, FMU_Type *base, uint32_t start, uint32_t end, uint32_t *seed, uint32_t *signature)
{
    assert(BOOTLOADER_API_TREE_POINTER);
    status_t status;
    status = BOOTLOADER_API_TREE_POINTER->flashDriver->flash_read_into_misr(config, base, start, end, seed, signature);
    ...
    flash_config_t *config, FMU_Type *base, uint32_t start, uint32_t end, uint32_t *seed, uint32_t *signature)
{
    assert(BOOTLOADER_API_TREE_POINTER);
    status_t status;
    status = BOOTLOADER_API_TREE_POINTER->flashDriver->ifr_read_into_misr(config, base, start, end, seed, signature);
